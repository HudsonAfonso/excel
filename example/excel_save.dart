...
void main() async {
  ...
  final listDynamic = (List<List<dynamic>>.generate(5, (_) => List<int>.generate(5, (i) => i + 1))
    ..insert(0, ['A', 'B', 'C', 'D', 'E']));

  for (var row = 0; row < listDynamic.length; row++) {
    for (var column = 0; column < listDynamic[row].length; column++) {
      final cellIndex = CellIndex.indexByColumnRow(columnIndex: column, rowIndex: row);
      final colorList = List.of(ExcelColor.values);
      final border = Border(borderColorHex: (colorList..shuffle()).first, borderStyle: BorderStyle.Thin);

      final string = listDynamic[row][column].toString();

      final cellValue = int.tryParse(string) != null ? IntCellValue(int.parse(string)) : TextCellValue(string);
