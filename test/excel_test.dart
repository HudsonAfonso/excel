...
void main() {
  ...
  test('Cell Data-Types from Microsoft Excel 365 Destkop', () {
    final file = './test/test_resources/dataTypesUsingMsExcel365Desktop.xlsx';
    final bytes = File(file).readAsBytesSync();
    final excel = Excel.decodeBytes(bytes);
    expect(excel.tables['Tabelle1']!.rows[2][1]?.value, equals(TextCellValue('Some text')));
    expect(excel.tables['Tabelle1']?.rows[3][1]?.value, equals(const IntCellValue(42)));
    expect(excel.tables['Tabelle1']?.rows[4][1]?.value, equals(const DoubleCellValue(12.3)));
    expect(excel.tables['Tabelle1']?.rows[5][1]?.value, equals(const DateCellValue(year: 2023, month: 4, day: 20)));
    expect(
      excel.tables['Tabelle1']?.rows[6][1]?.value,
      equals(const DateTimeCellValue(year: 2023, month: 4, day: 20, hour: 15, minute: 44, second: 13)),
    );
    expect(excel.tables['Tabelle1']?.rows[7][1]?.value, equals(const BoolCellValue(true)));
    expect(excel.tables['Tabelle1']?.rows[8][1]?.value, equals(const BoolCellValue(false)));
    expect(excel.tables['Tabelle1']?.rows[9][1]?.value, equals(const DoubleCellValue(15.99)));
    expect(excel.tables['Tabelle1']?.rows[10][1]?.value, equals(const DoubleCellValue(0.05)));
    expect(excel.tables['Tabelle1']?.rows[11][1]?.value, equals(const TimeCellValue(hour: 2, minute: 20, second: 10)));
  });

  test('Cell Data-Types from Google Spreadsheet', () {
    final file = './test/test_resources/dataTypesUsingGoogleSpreadsheet.xlsx';
    final bytes = File(file).readAsBytesSync();
    final excel = Excel.decodeBytes(bytes);
    expect(excel.tables['Sheet1']?.rows[2][1]?.value, equals(TextCellValue('Some text')));
    expect(excel.tables['Sheet1']?.rows[3][1]?.value, equals(const IntCellValue(42)));
    expect(excel.tables['Sheet1']?.rows[4][1]?.value, equals(const DoubleCellValue(12.3)));
    expect(excel.tables['Sheet1']?.rows[5][1]?.value, equals(const DateCellValue(year: 2023, month: 4, day: 20)));
    expect(
      excel.tables['Sheet1']?.rows[6][1]?.value,
      equals(const DateTimeCellValue(year: 2023, month: 4, day: 20, hour: 15, minute: 44, second: 13)),
    );
    expect(excel.tables['Sheet1']?.rows[7][1]?.value, equals(const BoolCellValue(true)));
    expect(excel.tables['Sheet1']?.rows[8][1]?.value, equals(const BoolCellValue(false)));
    expect(excel.tables['Sheet1']?.rows[9][1]?.value, equals(const DoubleCellValue(15.99)));
    expect(excel.tables['Sheet1']?.rows[10][1]?.value, equals(const DoubleCellValue(0.05)));
  });

  test('Cell Data-Types from LibreOffice', () {
    final file = './test/test_resources/dataTypesUsingLibreoffice.xlsx';
    final bytes = File(file).readAsBytesSync();
    final excel = Excel.decodeBytes(bytes);
    expect(excel.tables['Sheet1']?.rows[2][1]?.value, equals(TextCellValue('Some text')));
    expect(excel.tables['Sheet1']?.rows[3][1]?.value, equals(const IntCellValue(42)));
  ...
  test('Read/Write various data types', () async {
    final file = './test/test_resources/dataTypesUsingMsExcel365Desktop.xlsx';
    final bytes = File(file).readAsBytesSync();
    final excel = Excel.decodeBytes(bytes);
    {
      final sheet = excel.tables['Tabelle1']!;
      sheet.updateCell(CellIndex.indexByString('B4'), const DoubleCellValue(13.37));
      sheet.updateCell(CellIndex.indexByString('B5'), const DateCellValue(year: 2025, month: 11, day: 28));
      sheet.updateCell(CellIndex.indexByString('B6'), null);
      sheet.updateCell(CellIndex.indexByString('B7'), const TimeCellValue(hour: 20, minute: 15));
      sheet.updateCell(
    ...
    final bytesWritten = await excel.encode();
    final excelAgain = Excel.decodeBytes(bytesWritten!);
    {
      ...
      expect(b10.cellStyle?.numberFormat, equals(const CustomNumericNumFormat(formatCode: r'0\m\²')));
    }
  });

  test('Testing customNumFormats', () async {
    final excel = Excel.createExcel();
    final sheet = excel['Sheet1'];
    final format1 = const CustomNumericNumFormat(formatCode: '0.00%');
    final format2 = const CustomNumericNumFormat(formatCode: '#,##0.00');
    final styleA1 = CellStyle(numberFormat: format1);
    final styleB1 = CellStyle(numberFormat: format2);

    sheet.updateCell(CellIndex.indexByString('A1'), const DoubleCellValue(0.15), cellStyle: styleA1);
    sheet.updateCell(CellIndex.indexByString('B1'), const DoubleCellValue(123456.789), cellStyle: styleB1);
  ...
  group('Sheet Operations', () {
    ...
    final excel = Excel.decodeBytes(bytes);
    test('create Sheet', () {
      final sheetObject = excel['SheetTmp'];
  ...
  test('Saving XLSX File', () async {
    final file = './test/test_resources/example.xlsx';
    final bytes = File(file).readAsBytesSync();
    final excel = Excel.decodeBytes(bytes);
    final sheetObject = excel.tables['Sheet1']!;
    ...
    final newFile = './tmp/exampleOut.xlsx';
    final newFileBytes = File(newFile).readAsBytesSync();
  ...
  test('Saving XLSX File with appendRow', () async {
    final excel = Excel.createExcel();
    final sheet = excel['Sheet1'];
    ...
    sheet.appendRow([
      ...
      const DateTimeCellValue(year: 2023, month: 4, day: 20, hour: 15, minute: 44, second: 13),
      TextCellValue('value'),
    ]);
    ...
    expect(newExcel.tables['Sheet1']!.rows[0][0]!.value, equals(const IntCellValue(8)));
    expect(
      newExcel.tables['Sheet1']!.rows[0][0]!.cellStyle?.numberFormat.toString(),
      equals(NumFormat.defaultNumeric.toString()),
    );
    expect(newExcel.tables['Sheet1']!.rows[0][1]!.value, const DoubleCellValue(999.62221));
    expect(
      newExcel.tables['Sheet1']!.rows[0][1]!.cellStyle?.numberFormat.toString(),
      equals(NumFormat.defaultFloat.toString()),
    );
    expect(newExcel.tables['Sheet1']!.rows[0][2]!.value, const DateCellValue(year: 2023, month: 4, day: 20));
    expect(
      newExcel.tables['Sheet1']!.rows[0][2]!.cellStyle?.numberFormat.toString(),
    ...
    expect(
      newExcel.tables['Sheet1']!.rows[0][3]!.cellStyle?.numberFormat.toString(),
      equals(NumFormat.defaultDateTime.toString()),
    );
    expect(newExcel.tables['Sheet1']!.rows[0][4]!.value, TextCellValue('value'));
    expect(
      newExcel.tables['Sheet1']!.rows[0][4]!.cellStyle?.numberFormat.toString(),
  ...
  test('Saving XLSX File with superscript', () async {
    ...
    final newFile = './tmp/superscriptExampleOut.xlsx';
    final newFileBytes = File(newFile).readAsBytesSync();
    final newExcel = Excel.decodeBytes(newFileBytes);
    // delete tmp folder
    Directory('./tmp').delete(recursive: true);
  ...
  test(
    ...
    () async {
      ...
      final sharedStringsArchive = archive.findFile('xl/sharedStrings.xml')!;

      final oldSharedStringsDocument = XmlDocument.parse(utf8.decode(sharedStringsArchive.content));
      final oldCount = oldSharedStringsDocument.findAllElements('sst').first.getAttributeNode('count');
      final oldUniqueCount = oldSharedStringsDocument.findAllElements('sst').first.getAttributeNode('uniqueCount');
      ...
      final newArchive = ZipDecoder().decodeBytes(newFileBytes);
      final newSharedStringsArchive = newArchive.findFile('xl/sharedStrings.xml')!;

      final newSharedStringsDocument = XmlDocument.parse(utf8.decode(newSharedStringsArchive.content));
      final newCount = newSharedStringsDocument.findAllElements('sst').first.getAttributeNode('count');
      final newUniqueCount = newSharedStringsDocument.findAllElements('sst').first.getAttributeNode('uniqueCount');
  ...
  group('Header/Footer', () {
    test('Update header/footer', () async {
      final file = './test/test_resources/example.xlsx';
      final bytes = File(file).readAsBytesSync();
      final excel = Excel.decodeBytes(bytes);
      final sheetObject = excel.tables['Sheet1']!;
    ...
    test('Clone header/footer of existing Workbook', () {
      ...
      final testSheet = excel.tables['test_sheet'];
    ...
    test('Reader headerFooter attributes', () {
      final file = './test/test_resources/headerFooter.xlsx';
      final bytes = File(file).readAsBytesSync();
      final excel = Excel.decodeBytes(bytes);
      final sheetObject = excel.tables['Sheet1']!;

      final headerFooter = sheetObject.headerFooter!;
  ...
  group('Borders', () {
    ...
    test('test support for merged cells with borders', () {
      ...
      for (var i = 1; i < borderStyles.length; ++i) {
        final cellIndexStart = CellIndex.indexByString('B${4 * i + 2}');
        final cellIndexEnd = CellIndex.indexByString('D${4 * i + 4}');
        ...
        for (var j = cellIndexStart.rowIndex; j <= cellIndexEnd.rowIndex; j++) {
          for (var k = cellIndexStart.columnIndex; k <= cellIndexEnd.columnIndex; k++) {
            ...
            final borderStyle = Border(borderStyle: borderStyles[i], borderColorHex: 'FF000000'.excelColor);
    ...
    test('saving XLSX File with borders', () async {
      ...
      final sheetObject = newExcel.tables['Sheet1']!;
  ...
  group('Cell Style', () {
    test('read file with rich text', () {
      ...
      final cellA1 = sheetObject.cell(CellIndex.indexByString('A1')).value as TextCellValue;
      ...
      final cellA2 = sheetObject.cell(CellIndex.indexByString('A2')).value as TextCellValue;
      ...
      final cellA3 = sheetObject.cell(CellIndex.indexByString('A3')).value as TextCellValue;
  ...
  group('.xls file handling', () {
    ...
    test('Sheet Remove and Rename Operations', () {
      final excelFiles = List<Excel>.generate(5, (_) => Excel.createExcel());

      final data = List<List<int>>.generate(5, (x) => List<int>.generate(5, (i) => (x + 1) * (i + 1)));
      ...
      for (final element in excelFiles) {
        expect(element.getDefaultSheet(), defaultSheetName);
        for (var row = 0; row < data.length; row++) {
          for (var column = 0; column < data[row].length; column++) {
            final border = Border(borderColorHex: borderColor[column], borderStyle: BorderStyle.Thin);
        ...
        for (var row = 0; row < data.length; row++) {
          for (var column = 0; column < data[row].length; column++) {
            final cell = element.tables[newName]?.rows[row][column];
  ...
  group('Spanned Items', () {
    test('read spanned items', () async {
      final file = './test/test_resources/spannedItemExample.xlsx';
      final bytes = File(file).readAsBytesSync();
      final excel = Excel.decodeBytes(bytes);

      final sheet = excel.tables['Spanned Items']!;

      void testSpannedItemsSheetValues(Sheet sheet) {
      ...
      void testSpannedItemsList(Sheet sheet) {
        final spannedItems = sheet.spannedItems;
      ...
      final newFile = './tmp/spannedItemExampleOut.xlsx';
      final newFileBytes = File(newFile).readAsBytesSync();
      final newExcel = Excel.decodeBytes(newFileBytes);
      // delete tmp folder
      Directory('./tmp').delete(recursive: true);

      final newSheet = newExcel.tables['Spanned Items']!;
  ...
  test('Parse column width row height', () {
    final file = './test/test_resources/columnWidthRowHeight.xlsx';
    final bytes = File(file).readAsBytesSync();
    final excel = Excel.decodeBytes(bytes);
    final sheetObject = excel.tables['Sheet1']!;
  ...
  test('Decode customNumFmtIdBelow164.xlsx without throwing exception', () {
    final file = './test/test_resources/customNumFmtIdBelow164.xlsx';
